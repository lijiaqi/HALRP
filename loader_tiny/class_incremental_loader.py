...
from .idataset import _get_datasets, DummyDataset
...
class IncrementalLoader:
    ...
    def new_task(self, memory=None):
        ...
        sub_class_order = self.cpt_shuffled[self._current_task]
        min_class = min(sub_class_order)
        max_class = max(sub_class_order)
        x_train, y_train = self._select(
            self.data_train, self.targets_train, low_range=min_class, high_range=max_class + 1
        )
        y_train = self._map_new_class_index(y_train, sub_class_order)
        x_val, y_val = self._select(
            self.data_val, self.targets_val, low_range=min_class, high_range=max_class + 1
        )
        y_val = self._map_new_class_index(y_val, sub_class_order)
        x_test, y_test = self._select(
            self.data_test, self.targets_test, low_range=min_class, high_range=max_class + 1
        )
        y_test = self._map_new_class_index(y_test, sub_class_order)
        ...
        task_info = {
            ...
            "n_val_data": x_val.shape[0] if len(x_val) > 0 else None,
    ...
    def _setup_test_tasks(self, validation_split):
        self.test_tasks = []
        self.val_tasks = []
        for i in range(len(self.cpt_shuffled)):
            sub_class_order = self.cpt_shuffled[i]
            # print("sub_class_order={}".format(sub_class_order))
    ...
    def get_dataset_info(self):
        if self._opt.dataset == "tinyimagenet":
            ...
            n_inputs = (
                self.data_train.shape[3] * self.data_train.shape[1] * self.data_train.shape[2]
            )
            input_size = [
    ...
    def _setup_data(
        self, datasets, class_order_type=False, seed=1, increment=10, validation_split=0.0
    ):
        ...
        for dataset in datasets:
            ...
            self.task_order = list(range(len(self.increments)))
            random.seed(seed)
            random.shuffle(self.task_order)
            print("Task order:", self.task_order)
            self.cpt_ori = [order[i : i + increment] for i in range(0, len(order), increment)]
            self.cpt_shuffled = [self.cpt_ori[t] for t in self.task_order]
            print("Classes per task:", self.cpt_shuffled)
