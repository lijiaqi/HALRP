...
from data_loader_five import get_loader
import agents
...
parser.add_argument("--data_type", type=str, default="default", choices=["default", "superclass"])

parser.add_argument(
    "--model", type=str, default="AlexNet", choices=["LeNet", "AlexNet", "ResNet18", "MLP"]
)

parser.add_argument("--source_dir", type=str, default=".", help="The root folder of scripts")
parser.add_argument(
    "--data_dir", type=str, default="data", help="The root folder of dataset or downloaded data"
)


parser.add_argument(
    "--method_desc", type=str, default="", help="Method description added to the file prefix"
)
parser.add_argument("--folderName", type=str, default="five")
parser.add_argument("--log_dir", type=str, default="logs", help="dir for saving model logs")
parser.add_argument(
    "--tab_dir", type=str, default="accTable", help="dir for saving model performance table"
)
parser.add_argument(
...
parser.add_argument("--epoch_freq", type=float, default=1, help="Print the log at every x epochs")
parser.add_argument(
...
if not os.path.exists(args.tab_dir):
    os.mkdir(args.tab_dir)
if not os.path.exists(args.tune_dir):
    os.mkdir(args.tune_dir)
...
if args.fixed_order:
    file_prefix = "fixed_order_" + args.agent_name + "_" + args.method_desc + "_" + dt_string
else:
    file_prefix = (
...
with Tee(os.path.join(args.log_dir, file_prefix + ".log")):
    print("Split 5-Datasets Experiment information for the method:", args.agent_name)
    print(
        "SEED:",
        args.seed,
        "\nREPEAT:",
    ...
    for l1_hyp in l1_hyp_list:
        ...
        for reg_coef in reg_coef_list:
            ...
            for r in range(args.repeat):
                print("\n====== Prepare experiment repeats:", r + 1, "/", args.repeat, "======")
                # Prepare dataloaders
                train_loader_splits, valid_loader_splits, task_output_space = get_loader(
                    seed=args.seed,
                    loader_type="train",
                    fixed_order=False,
                ...
                print("!!!!!!!!!!! NUM OF TASKS", train_loader_splits)
                test_loader_splits = get_loader(
                    seed=args.seed,
                    loader_type="test",
                    batch_size=args.batch_size,
                    pc_valid=args.valid_size,
                ...
                if args.agent_name == "MTL":
                    ...
                    increase_size_history[l1_hyp][reg_coef][r, :] = agent.increase_size_rate
                    # Evaluate on current and previous validation set
                    print("===== Val_ACC (1st Row) and Test_ACC (2nd Row) on trained tasks =====")
                    val_timer.tic()
                    ...
                    print(
                        np.stack(
                            (
                                full_val_acc[l1_hyp][reg_coef][r, task_id, : (task_id + 1)],
                                full_test_acc[l1_hyp][reg_coef][r, task_id, : (task_id + 1)],
                            )
                        ).round(2)
                    )
                    if task_id > 0:
                        tab_forget = (
                            np.diag(
                                full_test_acc[l1_hyp][reg_coef][r, :task_id, :task_id].reshape(
                                    task_id, task_id
                                )
                            )
                    ...
                    for task_id in range(n_task):
                        ...
                        if task_id > 0:
                            ...
                            if args.agent_name in ["FHALRP", "FLRP"]:
                                for i_rank, (_, rank_l) in enumerate(agent.rank_current.items()):
                                    rank_history[l1_hyp][reg_coef][
                                        r, task_id, i_rank
                                    ] = rank_l.item()
                            elif args.agent_name in ["BHALRP", "BLRP"]:
                                for i_rank, (_, rank_l) in enumerate(agent.rank_previous.items()):
                                    rank_history[l1_hyp][reg_coef][
                                        r, task_id, i_rank
                                    ] = rank_l.item()
                        else:
                            agent.learn_batch(
                                train_loader_splits[task_id], task_id, valid_loader_splits[task_id]
                            )
                        ...
                        for eval_id in range(task_id + 1):
                            if "WSN" in args.agent_name:
                                ...
                                full_val_acc[l1_hyp][reg_coef][
                                    r, task_id, eval_id
                                ] = agent.validation(valid_loader_splits[eval_id], eval_id)["acc"]
                                full_test_acc[l1_hyp][reg_coef][
                                    r, task_id, eval_id
                                ] = agent.validation(test_loader_splits[eval_id], eval_id)["acc"]
                ...
                tab_forget = (
                    np.diag(full_test_acc[l1_hyp][reg_coef][r, :, :].reshape(n_task, n_task))
    ...
    if args.agent_name in ["HALRP"]:
        with open(os.path.join(args.tab_dir, file_prefix + "_rank_" + ".dict"), "wb") as table_file:
