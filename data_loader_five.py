...
def get_loader(
        ...
        for n, idx in enumerate(idata):
            if idx == 0:
                ...
                for s in ["train", "test"]:
                    ...
                    for image, target in loader:
                        image = image.expand(
                            1, 3, image.size(2), image.size(3)
                        )  # Create 3 equal channels
                        data[n][s]["x"].append(image)
                        data[n][s]["y"].append(target.numpy()[0])
                ...
                std = (0.3318,)
                dat = {}
                dat["train"] = FashionMNIST(
                    "./data/Five_data/fashion_mnist",
                    train=True,
                    download=True,
                    transform=transforms.Compose(
                        [
            ...
            for s in ["train", "test"]:
                ...
                torch.save(
                    data[n][s]["x"],
                    os.path.join(
                        os.path.expanduser("./data/Five_data/binary_mixture_5_Data"),
    ...
    for t in data.keys():
        train_datasets[t] = single_handler(X=data[t]["train"]["x"], Y=data[t]["train"]["y"])
...
class notMNIST(torch.utils.data.Dataset):
    ...
    def __init__(self, root, train=True, transform=None, download=False):
        ...
        self.url = (
            "https://github.com/nkundiushuti/notmnist_convert/blob/master/notmnist.zip?raw=true"
        )
