...
from models.resnet import ResNet_flat_18, rankPerturb4_Res18


class HALRP_Res(nn.Module):
    def __init__(self, agent_config):
        super(HALRP_Res, self).__init__()
    ...
    def save_model_task(self, task_id, epoch):
        ...
        filename = os.path.join(
            dir_save, "FHALRP" + "_task" + str(task_id) + "_e" + str(epoch + 1) + ".pth"
        )
        ...
        if task_id == 0:
            ...
            task_state = {
                ...
                "WMask_Sperturb_alltask": self.WMask_Sperturb_alltask[
                    task_id
                ].state_dict(),
                "WMask_Rperturb_alltask": self.WMask_Rperturb_alltask[
                    task_id
                ].state_dict(),
                "WBias_Sperturb_alltask": self.WBias_Sperturb_alltask[
                    task_id
                ].state_dict(),
                "WBias_Rperturb_alltask": self.WBias_Rperturb_alltask[
                    task_id
                ].state_dict(),
                "task_id": task_id,
            }
    ...
    def create_model(self, task_id, track_bn_stats=True):
        if "img_sz" in self.config:
            img_sz = self.config['img_sz']
        else:
            img_sz = 32
        if self.config['model'] == 'ResNet18':
            model = ResNet_flat_18(out_dim=self.config['out_dim'][task_id],
                                   n_task=len(self.config['out_dim']), 
    ...
    def update_model_newtask(self, task_id):
        if task_id > 0:
            i = task_id
            if self.config['model'] == 'ResNet18':
                self.model_alltask[str(i)] = rankPerturb4_Res18(
    ...
    def new_task(self, task_id):
        ...
        for i in range(len(self.model.features)):
            ...
            if isinstance(
                self.model.features[i],
                (torch.nn.modules.BatchNorm2d, torch.nn.modules.BatchNorm1d),
            ):
    ...
    def estimate_ranks(self, dataloader, task_id, approxiRate=0.6, val_loader=None):
        ...
        for k, p in self.param_track.items():
            ...
            if isinstance(self.model.features[int(k)], torch.nn.modules.conv.Conv2d):
                w_free = self.model.features[int(k)].weight.detach().clone()
        ...
        for i, (k, p) in enumerate(self.param_track.items()):
            ...
            if isinstance(self.model.features[int(k)], torch.nn.modules.conv.Conv2d):
                ...
                w_base = p.detach().clone()
                discrepancy = w_free - \
                    maskS_dict[k].view(1, -1, 1, 1) * w_base * maskR_dict[k].view(-1, 1, 1, 1)
                discrepancy_reshape = discrepancy.view(wShape[0], wShape[1],
                                                       wShape[2] * wShape[3]).permute(2, 0, 1)
                ...
                w_base = p.detach().clone()
                discrepancy = w_free - \
                    maskS_dict[k].view(1, -1) * w_base * maskR_dict[k].view(-1, 1)
        ...
        for i, (k, p) in enumerate(self.param_track.items()):
            if isinstance(self.model.features[int(k)], torch.nn.modules.conv.Conv2d):
                scale = p.shape[2] * p.shape[3] # ks_h * ks_w
            else:
                scale = 1
            idx_start = idx_start + (0 if i == 0 else num_p_layer[i - 1].int())
            idx_end = idx_end + num_p_layer[i].int()
            imp_sv[idx_start:idx_end] = scale * \
                (s_dict[k]**2) * self.importance_alltask[task_id][int(k)] 

        imp_sv_desc, desc_index = imp_sv.sort(descending=True)
        cumRatio_imp_sv_desc = imp_sv_desc.cumsum(axis=0)
        ...
        for i, k in enumerate(self.param_track.keys()):
            if self.rankMethod == 'flat':
                ...
                self.rank_current[int(k)] = (
                    ...
                    & (desc_index[:(truncateR + 1)] >= cum_p_layer[i])).sum().int()
        ...
        for k in self.param_track.keys():
            ...
            WBias_Sperturb[k] = torch.nn.Parameter(v_dict[k][:, :rank].transpose(0, 1) *
                                                   s_dict[k][:rank].sqrt().view(-1, 1),
                                                   requires_grad=True)
            WBias_Rperturb[k] = torch.nn.Parameter(u_dict[k][:, :rank] *
                                                   s_dict[k][:rank].sqrt().view(1, -1),
                                                   requires_grad=True)
        ...
        if self.rankMethod == 'flat':
            ...
            self.log("The selected rank for each layer:",
    ...
    def criterion(self, preds, targets, regularization=True):
        ...
        if regularization:
            ...
            if self.trained_task == 0:
                for i in range(len(self.model.features)):
                    if isinstance(self.model.features[i],
                                  (torch.nn.modules.conv.Conv2d, torch.nn.modules.Linear)):
    ...
    def l1_pruning(self, pv, pmethod):
        self.log("Pruning method:", pmethod, "--Bound all tasks:",
                 self.config['prune_boundAlltask'])
        ...
        with torch.no_grad():
            if pmethod == 'absolute':
                ...
                if pmethod == 'relative':
                    self.log('Pruning task-adaptive param bias with value <=', round(rate, 3),
                ...
                if pmethod == 'mixAR':
                    self.log('Pruning task-adaptive param bias with value <=', value, '; and',
                             round(rate, 3), '-quantile of each layer...')
                elif pmethod == 'mixAR_AllLayer':
                    self.log('Pruning task-adaptive param bias with value <=', value, '; and',
                             round(rate, 3), '-quantile of all layers...')
                elif pmethod == 'mixAR_AllLayerAllTask':
                    self.log('Pruning task-adaptive param bias with value <=', value, '; and',
                             round(rate, 3), '-quantile of all layers of all tasks...')
            else:
            ...
            if pmethod in ["relativeAllLayerAllTask", "mixAR_AllLayerAllTask"]:
                ...
                for task_id in range(1, self.trained_task + 1):
                    for key in self.WBias_Rperturb_alltask[task_id].keys():
                        ...
                        param_AllLayerAllTask = param_AllLayerAllTask + (paramS.data.reshape(-1),
                ...
                for task_id in range(1, self.trained_task + 1):
                    ...
                    for key in self.WBias_Rperturb_alltask[task_id].keys():
                        ...
                        reduce_task.append(
                            round(
                                torch.le(
                                    torch.cat(
                                        (paramS.data.reshape(-1), paramR.data.reshape(-1))).abs(),
                                    threshold).double().mean().item(), 2))
                    ...
                    self.log("Prune task", task_id, "-- each layer reduced by rate:", reduce_task,
                             "by threshold:", round(threshold, 5))
                return None
                ...
                if pmethod in ['relativeAllLayer', 'mixAR_AllLayer']:
                    ...
                    for key in self.WBias_Sperturb_alltask[task_id].keys():
                        ...
                        if pmethod == "relative":
                            threshold = np.percentile(
                                torch.cat(
                                    (paramS.data.reshape(-1), paramR.data.reshape(-1))).abs().cpu(),
                                rate * 100)
                        elif pmethod == "mixAR":
                            threshold = np.percentile(
                                torch.cat(
    ...
    def learn_batch(self,
                    ...
                    preval_loader=None,
                    preval_id=None):
        assert task_id == (self.trained_task +
                           1), 'expect task_id to be {}'.format(self.trained_task + 1)
        ...
        for epoch in range(self.config['n_epochs']):
            ...
            for i, (inputs, targets) in enumerate(train_loader):
                ...
                if (epoch == 0 and i == 0) or ((epoch % epoch_freq) == 0 and print_freq > 0 and
                                               (i % print_freq) == (print_freq - 1) and i !=
                                               (len(train_loader) - 1)):
                    if val_loader != None:
            ...
            if (epoch % epoch_freq) == 0 or ((epoch + 1) == self.config['n_epochs']):
                if val_loader != None:
                    ...
                    if (acc_max <
                            val_acc) and (epoch >=
                                          (self.config['n_epochs'] - self.config['check_lepoch'])):
                        acc_max = val_acc
                        acc_max_idx = epoch
