...
class HALRP(nn.Module):
    def __init__(self, agent_config):
        ...
        self.rank_current = {}  # determine the ranks of perturbation for the current task
        self.weightedLoss_alltask = {}
        self.explainedRate_alltask = {}
        self.param_track = nn.ParameterDict()  # track the base weights
        ...
        if agent_config["gpuid"][0] >= 0:
            ...
            self.gpu = False
        self.init_optimizer()
        self.trained_task = -1  # progress of the trained task id
    ...
    def create_model(self, task_id):
        ...
        model = AlexNet(out_dim=self.config["out_dim"][task_id])
        return model

    def update_model_newtask(self, task_id):
        if task_id > 0:
            i = task_id
            self.model_alltask[str(i)] = rankPerturb4_AlexNet(
    ...
    def new_task(self, task_id):
        ...
        # Save new model task-specific component
        ...
        # Initialize paramters from the base model
        self.model.features.load_state_dict(self.model_alltask[str(0)].features.state_dict())

        if self.gpu:
            self.cuda()

    # Estiamte ranks and generate optimal low-rank pertubation by SVD
    def estimate_ranks(self, dataloader, task_id, approxiRate=0.6, val_loader=None):
        ...
        for k, p in self.param_track.items():
            ...
            if isinstance(self.model.features[int(k)], torch.nn.modules.conv.Conv2d):
                ...
                )  # parameters of unconstrained model
                w_base = p.detach().clone()
                for j in range(wShape[0]):
                    maskR_dict[k][j] = (w_free[j, :, :, :] * w_base[j, :, :, :]).sum() / (
                        w_base[j, :, :, :] ** 2
                    ).sum()
                for i in range(wShape[1]):
                    maskS_dict[k][i] = (
                        maskR_dict[k] * (w_free[:, i, :, :] * w_base[:, i, :, :]).sum((1, 2))
                    ).sum() / (maskR_dict[k] * (w_base[:, i, :, :] ** 2).sum((1, 2))).sum()
                ...
                for i in range(wShape[1]):
                    maskS_dict[k][i] = (maskR_dict[k] * (w_free[:, i] * w_base[:, i])).sum() / (
                        maskR_dict[k] * (w_base[:, i] ** 2)
                    ).sum()
        # Get the best low rank approximations (SVD) to the remain ( B )
        u_dict = {}
        s_dict = {}
        v_dict = {}
        ...
        # Initialize low-rank perturbation parameters for Conv and FC
        ...
        # Update the parameter task list
    ...
    # Compute the laywise aggregated fisher information of (FC,conv weights)
    # which is the average trace of the layerwise fisher
    def calculate_importance(self, dataloader):
        self.log("Computing Layerwise Weight Importance...")
        imp_timer = Timer()
        imp_timer.tic()
        # Initialize the importance dictionary and track the corresponding weights
        importance = OrderedDict()  # {} if python>=3.7
        ...
        for inputs, targets in dataloader:
            ...
            # Layerwise importance as hessian layer block average trace
            for i, p in importance.items():
                p += torch.mean(self.param_track[str(i)].grad ** 2) * len(inputs) / len(dataloader)
            #   p += torch.sum(self.param_track[str(i)].grad ** 2 ) * len(inputs) / len(dataloader)
    ...
    def criterion(self, preds, targets, regularization=True):
        # The inputs and targets should come from a single task
    ...
    def l1_pruning(self, pv, pmethod):
        ...
        with torch.no_grad():
            ...
            # Prune applied to the all trained tasks
    ...
    def learn_batch(
        ...
        if task_id > 0:
            ...
            self.trained_task += 1
            self.init_optimizer(lowRank=False)  # Reset optimizer
            # Pretrain without constraints and estiamte ranks and generate optimal pertubation
            self.estimate_ranks(train_loader, task_id, self.approxiRate, val_loader)
            self.init_optimizer(lowRank=True)  # Reset optimizer for the rank perturbed model
        elif task_id == 0:
            self.trained_task += 1  # Add trained task count before training

        if task_id == 1:
            self.config["n_epochs"] = int(self.config["n_epochs"] - self.config["estRank_epoch"])

        # Update the new task model reference
        ...
        if task_id > 0:
            ...
            self.increase_size_rate = round(self.count_parameter() / self.baseSize - 1, 3)
            self.log("Increased parameters so far over base model:", self.increase_size_rate)
        # self.previousSize = self.count_parameter()
        ...
        with torch.no_grad():
            for batch_index, (inputs, targets) in enumerate(dataloader):
                if self.gpu:
                    ...
                    targets = targets.cuda()
                outputs = self.forward(inputs, task_id)
                # Summarize the performance of the given task data loader
